...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    x=torch.arange(y,stop+1,10, dtype=torch.float64)
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y=x.view(-1,4)[[0,3,1,4,2,5]].view(3,8)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    esx=((x*x)/x.shape[0]).sum(dim=0, keepdim=True)
    ex=(x/x.shape[0]).sum(dim=0,keepdim=True)
    sdv=((esx-ex*ex)*x.shape[0]/(x.shape[0]-1)).sqrt()
    z=(x-ex)/sdv
    y=z.to(x)
