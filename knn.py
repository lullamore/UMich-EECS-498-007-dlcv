...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            x=torch.reshape(x_train[i],(-1,))
            y=torch.reshape(x_test[j],(-1,))
            
            dists[i][j]=(x-y).pow(2).sum().sqrt()
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        x=torch.reshape(x_train[i],(-1,))
        y=torch.reshape(x_test,(x_test.shape[0],-1))
            
        dists[i]=(x-y).pow(2).sum(dim=1).sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x=torch.reshape(x_train,(x_train.shape[0],-1))
    y=torch.reshape(x_test,(x_test.shape[0],-1)).t()

    dists=((x.pow(2).sum(dim=1,keepdim=True)+y.pow(2).sum(dim=0,keepdim=True)-2*torch.matmul(x,y))).sqrt()
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        ...
        y_pred[i]=torch.argmax(torch.bincount(y_train[indices]))

    y_pred.to(dtype=torch.int64)
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.dists=compute_distances_no_loops(self.x_train, x_test)

        y_test_pred=predict_labels(self.dists, self.y_train, k)
...
def knn_cross_validate(
    ...
    y_train_folds = torch.chunk(y_train.reshape(y_train.shape[0],-1),num_folds)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            x_train_i=torch.zeros(0,)
            y_train_i=torch.zeros(0,)
            for j in range(num_folds):
                if i!=j:
                    x_train_i=torch.cat((x_train_i,x_train_folds[j]))
                    y_train_i=torch.cat((y_train_i,y_train_folds[j].view(-1)))


            model=KnnClassifier(x_train_i,y_train_i)
            accuracy=model.check_accuracy(x_train_folds[i],y_train_folds[i].view(-1),k)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies:
        if sum(k_to_accuracies[k])>best_score:
            best_score=sum(k_to_accuracies[k])
            best_k=k
